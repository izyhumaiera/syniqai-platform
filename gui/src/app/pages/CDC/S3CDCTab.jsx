...
import { 
  ...
  Activity, Clock, Server, TrendingUp, FileJson, Loader
} from 'lucide-react';
...
export default function S3CDCTab({ domain, autoRefresh, onRefreshed }) {
  ...
  const fetchStatus = async () => {
    try {
      const response = await axios.get(`${API_BASE}/cdc/s3/status`);
      setCdcStatus(response.data);
      setError(null);
    } catch (err) {
      console.error('Failed to fetch S3 CDC status:', err);
  ...
  const fetchEvents = async () => {
    try {
      const response = await axios.get(`${API_BASE}/cdc/events`, {
        params: { source: 's3', limit: 20 }
      });
      setEvents(response.data.events || []);
    } catch (err) {
      console.error('Failed to fetch S3 events:', err);
    }
  };
  ...
  const handleStart = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE}/cdc/s3/start`, {});
      alert(`? S3 CDC started!\nPID: ${response.data.pid}\n\nEvents will start flowing to Kafka.`);
      await refreshAll();
    } catch (err) {
      const msg = err.response?.data?.detail || err.message;
      alert(`? Failed to start S3 CDC:\n\n${msg}`);
    } finally {
  ...
  const handleStop = async () => {
    if (!confirm('?? Stop S3 CDC?\n\nThis will stop capturing S3 file events.')) {
      return;
    }
    
    setActionInProgress(true);
    try {
      const response = await axios.post(`${API_BASE}/cdc/s3/stop`);
      alert(`? S3 CDC stopped\n\n${response.data.message}`);
      await refreshAll();
    } catch (err) {
      const msg = err.response?.data?.detail || err.message;
      alert(`? Failed to stop S3 CDC:\n\n${msg}`);
    } finally {
      setActionInProgress(false);
    }
  };
    ...
    <div className="space-y-6">
        ...
        <div className={`bg-white rounded-lg border-2 ${cdcStatus?.running ? 'border-green-200' : 'border-gray-200'} p-4`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <Cloud className={`w-5 h-5 ${cdcStatus?.running ? 'text-green-600' : 'text-gray-400'} mr-2`} />
              <span className="text-sm font-semibold text-gray-700">S3 CDC Poller</span>
            </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 mb-1">
              AWS S3 Change Data Capture
            </h3>
            <p className="text-sm text-gray-600">
              Poll AWS S3 buckets for new and updated files, stream them to Bronze layer via Kafka
            </p>
          </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg p-4 border border-orange-200">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-orange-900">{eventsToday.toLocaleString()}</div>
              <div className="text-sm text-orange-700 font-medium">Events Today</div>
            </div>
        ...
        <div className="overflow-x-auto">
          {loading && events.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-gray-500">
              ...
              <p className="text-lg font-medium">No S3 events yet</p>
              <p className="text-sm">Start CDC to begin capturing S3 file events</p>
            </div>
          ) : (
                  ...
                  <tr key={idx} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 py-1 rounded text-xs font-semibold ${
                        event.operation === 'create' ? 'bg-green-100 text-green-700' :
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      {event.file_size ? `${(event.file_size / 1024).toFixed(1)} KB` : 'N/A'}
                    </td>
